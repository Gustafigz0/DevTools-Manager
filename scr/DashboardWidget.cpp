...
void DashboardWidget::createChartSection() {
    ...
    chartView_->setViewportUpdateMode(QGraphicsView::MinimalViewportUpdate);
    chartView_->setOptimizationFlag(QGraphicsView::DontAdjustForAntialiasing, true);
    chartView_->setOptimizationFlag(QGraphicsView::DontSavePainterState, true);
    chartView_->setOptimizationFlag(QGraphicsView::DontClipPainter, true);
    chartView_->setMinimumHeight(400);
        chartView_->setFocusPolicy(Qt::StrongFocus);
    chartView_->setStyleSheet("border: none; background: transparent;");
    
    // Empty state label
    emptyStateLabel_ = new QLabel("📊\ngathering information, it may take a while", this);
...
void DashboardWidget::updateData(const QVector<Product>& products) {
    // Compute signature to detect real changes and avoid unnecessary redraws
    const quint64 newSig = computeProductsSignature(products);
    if (newSig == lastProductsSignature_) {
        // No data changes; only refresh timestamp
        updateLastRefreshTime();
        return;
    }

    products_ = products;
    lastProductsSignature_ = newSig;
...
void DashboardWidget::performUpdate() {
    // Batch UI updates to minimize repaints
    this->setUpdatesEnabled(false);
    if (chartView_) chartView_->setUpdatesEnabled(false);

    updateStatistics();
    updateChart();
    updateLastRefreshTime();

    if (chartView_) chartView_->setUpdatesEnabled(true);
...
void DashboardWidget::updatePieChart() {
    ...
    for (int i = 0; i < sortedProducts.size(); ++i) {
        ...
        if (sliceCount < MAX_PIE_SLICES) {
            ...
            // Avoid hover/explode overhead for performance
...
quint64 DashboardWidget::computeProductsSignature(const QVector<Product>& products) const {
    // Lightweight signature: combine count, total qty, total price (in cents), and name hash sum
    const int count = products.size();
    long long totalQty = 0;
    long long totalPriceCents = 0;
    quint64 nameHashSum = 0;
    for (const Product& p : products) {
        totalQty += p.getQuantity();
        totalPriceCents += static_cast<long long>(std::llround(p.getPrice() * 100.0));
        // Simple name hash accumulation
        nameHashSum ^= qHash(p.getName());
        nameHashSum = (nameHashSum << 7) | (nameHashSum >> (57));
    }
    quint64 sig = static_cast<quint64>(count & 0xFFFF);
    sig = (sig << 16) ^ static_cast<quint64>(totalQty & 0xFFFFFFFF);
    sig ^= (static_cast<quint64>(totalPriceCents) & 0xFFFFFFFFFFFFULL);
    sig ^= nameHashSum;
    return sig;
}
