...
void MainWindow::createProductCard(const Product& product)
{
    ...
    selectBox->setFixedSize(24, 24);
    selectBox->setChecked(selectedProductIds_.contains(product.getId()));
    selectBox->setStyleSheet(R"(
        QCheckBox::indicator {
            width: 22px; height: 22px;
            border-radius:11px;
            border:2px solid #58abfa; background: #222;
        }
        QCheckBox::indicator:checked {
            background-color: #58abfa;
            border: 2px solid #58abfa;
        }
        QCheckBox { spacing: 0px; }
    )");
    connect(selectBox, &QCheckBox::toggled, this, [this, product](bool checked){
        onSelectProductToggled(product.getId(), checked);
    });
    cardLayout->addWidget(selectBox, 0, Qt::AlignVCenter);
    ...
    stockStatus->setStyleSheet(product.isInStock() ? "color: #2ecc71; font-size: 13px; background: transparent; min-width:108px;" : "color: #e74c3c; font-size: 13px; background: transparent; min-width:108px;");
